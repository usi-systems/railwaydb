...
Each of these variables serve an important purpose:
\begin{itemize}
\item $x$s define the attribute to partition assignment.
\item $y$s help formulate the objective function's I/O contributions
that are on a per-partition basis.
\item $z$s help formulate the objective function's I/O contributions
that are on a per-attribute basis.
...
function, that is the total query I/O cost:
\begin{eqnarray}
...
In Eq.~\ref{eq:no-obj}, we simply sum for each query and each partiton to add
the I/O cost of reading in the structural information found in a sub-block, if
the partition is used by the query. We then sum over each attribute as well,
and add the I/O cost of reading in the attributes. Note that $z_{a,p,q}$ could
have been  replaced with $x_{a,p}\cdot y_{p,q}$, but that would make the
objective function non-linear.

Our first constraint is that each attribute gets assigned to one and only one
partition. Formally:
\begin{eqnarray}
\forall_{a\in A}, \sum_{p=1}^{k} x_{a,p} = 1
\end{eqnarray}

Our second constraint is that, if a query contains an attribute assigned to a
partition, then that partition is used by the query. In essence, we want to
state: $\forall_{\{p,q\}\in [1..k]\times Q}, y_{p,q} = \mathbf{1}(\sum_{a\in
A} q(a)\cdot x_{a,p}>0)$. In order to formulate this we use the following ILP 
construction:
...
large constant guaranteed to be larger than $\beta_1$ for practical purposes:
\begin{eqnarray}
&& \beta_1 - \beta_2 \geq 0\nonumber\\
...
variable must be set to $1$. It must be $0$ in all other cases. That is, we
want: $\forall_{\{a,p,q\}\in A\times [1..k]\times Q},
z_{a,p,q}=\mathbf{1}(x_{a,p} = y_{p,q} = 1)$. We express this as a linear 
...
In Eq.~\ref{eq:no-z}, when the $x$ and $y$ variable are both $1$, the  $z$
...
corresponding $u$ variable must be set to $0$. It must be $0$ otherwise. In
other words,  we want $u_p = \mathbf{1}(\sum_{a\in A} x_{a,p}>0)$. This is
