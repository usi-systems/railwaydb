...
query workload, as observed by a disk block within a particular time range.

The partitioning of disk blocks into sub-blocks can be \emph{non-overlapping}
or \emph{overlapping}. In the non-overlapping case, each sub-block contains a
non-overlapping subset of the edge attributes. In the overlapping case, the
subset of attributes contained within sub-blocks can overlap. In both cases,
the complete graph structure for the block is replicated in the sub-blocks,
which is the main cause of the increased storage overhead.
