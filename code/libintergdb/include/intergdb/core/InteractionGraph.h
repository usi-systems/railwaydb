...
namespace intergdb { namespace core
{
    ...
    public:
        ...
        public:
            EdgeIterator(
                std::shared_ptr<typename HistoricalGraph::EdgeIterator> it)
                : it_(it)
            {}
            ...
            VertexId getToVertex()
            {
                return it_->getEdge().getToVertex();
            }

            Timestamp getTime()
            {
                return it_->getEdge().getTime();
            }

            std::shared_ptr<AttributeData> getEdgeData()
            {
                return it_->getEdgeData();
            }
    ...
    public:
        ...
        void processIntervalQueryBatch(IntervalQuery const & q,
        ...
        {
            return hisg_.getPartitionIndex();
        }

        size_t getEdgeIOCount() const
        {
            return hisg_.getEdgeIOCount();
        }

        size_t getEdgeReadIOCount() const
        {
            return hisg_.getEdgeReadIOCount();
        }

        size_t getEdgeWriteIOCount() const
        {
            return hisg_.getEdgeWriteIOCount();
        }

        Schema const & getVertexSchema()
        {
            return conf_.getVertexSchema();
        }

        Schema const & getEdgeSchema()
        {
            return conf_.getEdgeSchema();
        }

        Conf const & getConf() const
        {
            return conf_;
        }
