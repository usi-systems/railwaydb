...
namespace intergdb { namespace core
{
    ...
    public:
        ...
        {
        public:
            static std::auto_ptr<SpatialIndex::Region> getRegion(
                    Timestamp const start, Timestamp const end,
                    Timestamp yStart, Timestamp yEnd);
        public:
            IntervalData() {}
            IntervalData(BlockId id, VertexId vertex, Timestamp start, Timestamp end)
                : id_(id), vertex_(vertex), start_(start), end_(end) {}
            std::auto_ptr<SpatialIndex::Region> getRegion() const
            {
                return IntervalData::getRegion(start_, end_, 1.0, 9.0);
            }
            auto_array<unsigned char> getBytes(uint32_t & size) const;
            void load(SpatialIndex::IData const & data);
            BlockId const & getBlockId() const { return id_; }
            VertexId const & getVertex() const { return vertex_; }
            Timestamp const & getIntervalStart() const { return start_; }
            Timestamp const & getIntervalEnd() const { return end_; }
        private:
            BlockId id_;
            VertexId vertex_;
            Timestamp start_;
    ...
    public:
        ...
        private:
            ...
            public:
                ...
                void setCaller(caller_t & caller) { caller_ = &caller; }
                IntervalData const & getData() { return data_; }
            private:
                IntervalData data_;
                caller_t * caller_;
            };
    ...
    public:
        ...
        Iterator * getNewIterator(Timestamp const & start, Timestamp const & end)
    ...
    private:
        ...
        std::auto_ptr<SpatialIndex::ISpatialIndex> rtidx_;
    };
