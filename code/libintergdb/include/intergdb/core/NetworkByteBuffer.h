...
namespace intergdb { namespace core
{
    ...
    public:
        ...
        void addUInt16(uint16_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<uint16_t>(i).rawData,sizeof(uint16_t));
            icursor += sizeof(uint16_t);
        }
        uint16_t getUInt16()
        {
            uint16_t temp = network_to_host<uint16_t>(*reinterpret_cast<uint16_t *>(buf+ocursor)).data;
        ...
        void addInt16(int16_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<int16_t>(i).rawData,sizeof(int16_t));
            icursor += sizeof(int16_t);
        }
        int16_t getInt16()
        {
            int16_t temp = network_to_host<int16_t>(*reinterpret_cast<int16_t *>(buf+ocursor)).data;
        ...
        void addUInt32(uint32_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<uint32_t>(i).rawData,sizeof(uint32_t));
            icursor += sizeof(uint32_t);
        }
        uint32_t getUInt32()
        {
            uint32_t temp = network_to_host<uint32_t>(*reinterpret_cast<uint32_t *>(buf+ocursor)).data;
        ...
        void addInt32(int32_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<int32_t>(i).rawData,sizeof(int32_t));
            icursor += sizeof(int32_t);
        }
        int32_t getInt32()
        {
            int32_t temp = network_to_host<int32_t>(*reinterpret_cast<int32_t *>(buf+ocursor)).data;
        ...
        void addUInt64(uint64_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<uint64_t>(i).rawData,sizeof(uint64_t));
            icursor += sizeof(uint64_t);
        }
        uint64_t getUInt64()
        {
            uint64_t temp = network_to_host<uint64_t>(*reinterpret_cast<uint64_t *>(buf+ocursor)).data;
        ...
        void addInt64(int64_t i)
        {
            ...
            memcpy(buf+icursor, host_to_network<int64_t>(i).rawData,sizeof(int64_t));
            icursor += sizeof(int64_t);
        }
        int64_t getInt64()
        {
            int64_t temp = network_to_host<int64_t>(*reinterpret_cast<int64_t *>(buf+ocursor)).data;
        ...
        void addFloat(float f)
        {
            ...
            memcpy(buf+icursor, host_to_network<float>(f).rawData,sizeof(float));
            icursor += sizeof(float);
        }
        float getFloat()
        {
            float temp = network_to_host<float>(*reinterpret_cast<float *>(buf+ocursor)).data;
        ...
        void addDouble(double d)
        {
            ...
            memcpy(buf+icursor, host_to_network<double>(d).rawData,sizeof(double));
            icursor += sizeof(double);
        }
        double getDouble()
        {
            double temp = network_to_host<double>(*reinterpret_cast<double *>(buf+ocursor)).data;
    ...
    protected:
        ...
        {
            if (!autoDealloc)
                throw std::runtime_error(std::string("cannot resize fixed buffer"));
            uint64_t maxGrowth = 1ull + std::numeric_limits<uint32_t>::max() - size;
            uint64_t growth = extra;
            if (extra > maxGrowth)
                throw std::runtime_error(std::string("Buffer exceeds maximum size"));
            // testing whether we have to more than double the current buffer size
            if (extra > size) {
