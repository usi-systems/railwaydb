...
namespace intergdb { namespace core
{
    ...
    public:
        ...
        Candidate()
            : size_(sizeof(BlockId)), edgeCount_(0), outEdgeCount_(0)
        {}

        double getLocality(Conf::SmartLayoutConf::LocalityMetric metric) const;

        EdgeCounts const & getEdgeCounts() const
        {
            return numEdges_;
        }
        ...
        {
            return (numEdges_.count(headVertex)==0)
                ? 0 : numEdges_.find(headVertex)->second;
        }

        PresentVertices const & getPresentVertices() const
        {
            return presentVertices_;
        }

        Timestamp getNewestTime() const
        {
            return timestamps_.getTopItem().key();
        }
        ...
        size_t getNumDanglingEdges() const
        {
            return outEdgeCount_;
        }

        void setEdgeCount(ExpirationMap const & map,
                          VertexId headVertex, size_t n);

        bool hasReverseEdge(ExpirationMap const & map,
                            VertexId headVertex,
    ...
    private:
        ...
        // vertices that appear in neiglists
        std::unordered_map<VertexId, size_t> presentVertices_;
        // max timestamps of neiglists
        PriorityQueue<Timestamp, VertexId,
                      PriorityQueueOrder::MaxAtTop> timestamps_;
        // number of edges from each neiglists
        std::unordered_map<VertexId, size_t> numEdges_;
        // neiglist to neiglist edges
        std::unordered_map<SUEdge, size_t> internalEdges_;
    };
} } /* namespace */
