...
TEST_F(BasicReadWrite_test, WriteReadTest) 
{
    ...
    EXPECT_EQ(iqIt->getVertexData().getAttribute("v"), "v2");
    EXPECT_EQ(2, iqIt.getVertexId());
    iqIt.next();
    EXPECT_EQ(iqIt.isValid(), true);
    EXPECT_EQ(iqIt->getVertexData().getAttribute("v"), "v4");
    ...
    EXPECT_EQ(fiqIt.getEdgeData().getAttribute("a"), "a-data");
    EXPECT_EQ(fiqIt.getEdgeData().getAttribute("b"), "b-data");
