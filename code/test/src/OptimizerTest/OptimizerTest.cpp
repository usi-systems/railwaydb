...
TEST_F(OptimizerTest, nov_ex1)
{
    ...
    for (size_t i=0, iu=2; i<iu; ++i) {
        allAttributes.emplace_back(new Attribute(i, std::to_string(i), intergdb::common::Attribute::UNDEFINED));
    ...
    EXPECT_NE((*part1->getAttributes().begin())->getIndex(),
              (*part2->getAttributes().begin())->getIndex());
...
TEST_F(OptimizerTest, nov_ex2)
{
    ...
    for (size_t i=0, iu=2; i<iu; ++i) {
        allAttributes.emplace_back(new Attribute(i, std::to_string(i),
            intergdb::common::Attribute::UNDEFINED));
        workload.addAttribute(*allAttributes.back());
        stats.incrCountAndBytes(i, 8);
    }
    for (Attribute const * attribute : workload.getAttributes()) {
